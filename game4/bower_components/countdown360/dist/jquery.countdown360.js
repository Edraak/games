...
;(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    pause: function (cb) {
      if(paused){
        this.startedAt.setTime(new Date().getTime() - pausedTime);
        paused = false;
      }else{
        pausedTime = new Date().getTime() - this.startedAt.getTime();        
        //console.log(pausedTime);
        paused = true;
      }
      if (cb) { cb(); }
    },
    resetart: function (cb) {
      clearInterval(this.interval);
      this.settings.seconds = this.settings.seconds; 
      paused = false;
      this.start();     
    },
    ...
    _toHHMMSS: function (t) {
      return t;
    },

    _drawCountdownLabel: function (secondsElapsed) {
      this.ariaText.text(this._toHHMMSS(secondsLeft));
      ...
      this.pen.fillText(this._toHHMMSS(secondsLeft), x, y);
      if (drawLabel) {
